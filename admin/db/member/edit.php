            ...
            try {
                if (sizeof($_FILES) != null) {
                    if(isset($_POST['password'])){
                        $stmt = $con -> prepare('UPDATE member set name=:name, password=:password, birthDate=:birthDate,
                        phone=:phone, email=:email, address=:address, profileImg=:profileImg WHERE memberId=:memberId');
                        $password = base64_encode(hash('sha256',$_POST['password'],true));
                        $stmt->bindParam(':password',$password);
                    }else{
                        $stmt = $con -> prepare('UPDATE member set name=:name, birthDate=:birthDate,
                        phone=:phone, email=:email, address=:address, profileImg=:profileImg WHERE memberId=:memberId');
                    }
                    ...
                    if(isset($_POST['password'])){
                        $stmt = $con -> prepare('UPDATE member set name=:name, password=:password, birthDate=:birthDate,
                        phone=:phone, email=:email, address=:address WHERE memberId=:memberId');
                         $password = base64_encode(hash('sha256',$_POST['password'],true));
                        $stmt->bindParam(':password',$password);
                    }else{
                        $stmt = $con -> prepare('UPDATE member set name=:name, birthDate=:birthDate,
