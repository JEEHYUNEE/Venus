<?php
    ...
    if($title && $content){
        $sql = mq("insert into qna(title,content,hit,date,file,answerCheck) values('".$title."','".$content."','0','".$date."','".$filename."','N')"); 
        $sort1 = mq("alter table qna AUTO_INCREMENT=1;"); 
        $sort2 = mq("set @CNT = 0;"); 
        $sort3 = mq("update qna SET qna.qnaNum = @CNT:=@CNT+1;"); 
       
        echo "<script>alert('Q&A등록이 완료되었습니다.');opener.parent.location.reload();window.close();</script>";
    }else{
